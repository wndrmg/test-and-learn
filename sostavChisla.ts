 ...
 **/
function sostavChisla(massivChisel: number[], chislo: number):Array<Array<number>> {
    const sortedArr: number[] = JSON.parse(JSON.stringify(massivChisel)).sort();
    ...
    for(let i = 0; i < sortedArr.length; i++) {
        ...
        if (sum > chislo) {
            ...
            if (sum === chislo) {
                results.push(part.map((i) => sortedArr[i]));
            }
        }
    }
